...
class TicketHelper:
    ...
    def update_cache_ticket(self, ticket: Ticket) -> Ticket:
        ...
        cached_ticket = self.mongo.load(
            db_name=self.CACHE_DB_NAME, collection_name=self.COLLECTION_NAME, query=dict(user_id=ticket.user_id, username=ticket.username))
    ...
    def reset_cache_ticket(self, ticket: Ticket) -> Ticket:
        ...
        if tickets:
            result = self.mongo.delete_all(
                db_name=self.CACHE_DB_NAME, collection_name=self.COLLECTION_NAME, query=dict(user_id=ticket.user_id))
            if result is False:
